...
all_genres = movie_df.genres.apply(
    lambda x: " ".join(str(x).split("|"))
).values.tolist()  # split from |
all_genres = " ".join(
    set(all_genres)
).split()  # join all strings and break them into words
...
user_df["movieId"] = user_df["movieId"].astype(
    "int32"
)  # dont lower too much as it changes the numbers to accomodate to the range
...
def more_movies(idx, n: int = 10):
    ...
    return [
        movie_df[movie_df.movieId == dataset.idx2movieId[i.item()]]["title"].values[0]
