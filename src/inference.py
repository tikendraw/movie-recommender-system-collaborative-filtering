...
def more_movies(idx, movieId2idx, idx2movieId, movie_df, model, device, n=10):
    if isinstance(idx, int):
        new_movie_id = movieId2idx[idx]
        print(f'Movie: {get_movie_name(idx,movie_df)}')

    elif isinstance(idx, str):
        new_movie_id = int(get_movie_id(idx, movie_df))
        new_movie_id = movieId2idx[new_movie_id]
        print(f'Movie: {idx}')
    ...
    return [
        movie_df[movie_df.movieId == idx2movieId[i.item()]]['title'].values[0]
